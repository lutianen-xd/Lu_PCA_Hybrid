...
def run_PCA(activations_collect, key_idx, components, threshold=0.999):
    ...
    D_t = a.reshape(a_shape[0]*a_shape[1]*a_shape[2], a_shape[3])

    # PCA是无监督学习算法，fit 函数就是算法中的“训练”这一步骤，即用数据X来训练PCA模型。返回调用fit方法的对象本身
    pca.fit(D_t) #this should be N*H*W,M
    
    # 将数据X转换成降维后的数据。当模型训练好后，对于新输入的数据，都可用transform方法降维
    a_trans=pca.transform(D_t)
...
if __name__=='__main__':
    ...
    parser.add_argument('--data', action='store', default='data/CIFAR100/cifar-100-python',
            help='dataset path')
